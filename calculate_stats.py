...
def calculate_statistics(path, subsets, dataset_name, user_file ,sensitive_col ,output):
    ...
    for subset in subsets:
        ...
        for file_name in os.listdir(subset_path):
            if file_name.endswith(".inter"):
                ...
                s1 = new_df[new_df[sensitive_col] == 0].value_counts().sum() / new_df[sensitive_col].shape[0]
                s2 = new_df[new_df[sensitive_col] == 1].value_counts().sum() / new_df[sensitive_col].shape[0]
                ...
                stats.append({"Dataset" : dataset_name, "Subset ID" : number, "Is Filtered" : filtered, "Number of Users": V, "Number of Items": I, "Number of Ratings" : R, "Space Size" : space_size, "Shape" : shape, "Density" : density, "Rating per User" : R_per_user, "Rating per Item" : R_per_item,
                              "Gender == 0 Percentage" : s1, "Gender == 1 Percentage" :s2,
                              "Difference between Gender's Percentage": difference})

                stats_df = pd.DataFrame(stats)
                stats_df = stats_df.sort_values(by=["Is Filtered", "Subset ID"],
                                                key=lambda col: col if col.name != "Subset ID" else col.str.extract(
                                                    r'(\d+)$').iloc[:, 0].astype(int), ascending=[True, True])
...
calculate_statistics(base_path, subsets, dataset_name, user_file, sensitive_col, output_file)